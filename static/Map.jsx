...
var Map = React.createClass({
	propTypes: {
		...
		deleteMarkers: React.PropTypes.bool
	},
	...
	//Called before the next render - updates the "markers" array state
	componentWillReceiveProps: function(nextProps) {
		var currentMarkers = this.state.markers;
		if (nextProps.deleteMarkers) {
			this.deleteMarkers();
			currentMarkers = [];
		}
		var newMarkers = this.drawMarkers(nextProps.coordinates);
		this.setState({
			markers: currentMarkers.concat(newMarkers)
		});
	},

	//Called only once after initial render - map is stored in state so the component 
	//has the same map upon successive renders
